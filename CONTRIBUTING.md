...
The package declares `Language: en`, while `vignettes/bigbang-es.Rmd` is
intentionally written in Spanish. The high-level
`spelling::spell_check_package()` function has no per-file exclusion
...
Check the Spanish guide separately when the `es_ES` Hunspell dictionary
is installed:
...
The split is deliberate: adding every Spanish word to the English
WORDLIST would hide real spelling errors in the English documentation.
