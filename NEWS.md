...
- `create_metapackage(reexport = TRUE)` works. It previously failed for every
  ...
  the call aborted with "there is no package called 'pkg_1.0'" before reaching
  the writer that does the work. That block also declared
  `S3method(<name>, default)` for every export whether or not it was a generic.
  It was removed; `write_reexports_file()` already resolves component namespaces
  and distinguishes S3 generics from ordinary functions.
